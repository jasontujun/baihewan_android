...
public class BbsUrlUtil {
    ...
    public static String getTop10Url(){
        GlobalStateSource gss = (GlobalStateSource) DataRepo.getInstance().getSource(SourceName.GLOBAL_STATE);
        if(gss.getLoginStatus() == GlobalStateSource.LOGIN_STATUS_NO_LOGIN){
            return bbsHost + "bbstop10";
        }else {
            return bbsHost + "vd" + gss.getBbsCode() + "/bbstop10";
        }
    }
     ...
     * @return
     */
    public static String getBoardArticleListUrl(String boardName, int no, boolean isTheme){
        GlobalStateSource gss = (GlobalStateSource) DataRepo.getInstance().getSource(SourceName.GLOBAL_STATE);
        if(gss.getLoginStatus() == GlobalStateSource.LOGIN_STATUS_NO_LOGIN){
            if(isTheme){
            ...
            if(isTheme){
                return bbsHost + "vd" + gss.getBbsCode() + "/bbstdoc?board="+boardName+"&start="+no+"&type=doc";
            }else {
                return bbsHost + "vd" + gss.getBbsCode() + "/bbsdoc?board="+boardName+"&start="+no+"&type=doc";
            }
        }
    }
     ...
     * @return
     */
    public static String getBoardFirstPageUrl(String boardName, boolean isTheme){
        GlobalStateSource gss = (GlobalStateSource) DataRepo.getInstance().getSource(SourceName.GLOBAL_STATE);
        if(gss.getLoginStatus() == GlobalStateSource.LOGIN_STATUS_NO_LOGIN){
            if(isTheme){
            ...
            if(isTheme){
                return bbsHost + "vd" + gss.getBbsCode() + "/bbstdoc?board="+boardName+"&start=999999&type=doc";
            }else {
                return bbsHost + "vd" + gss.getBbsCode() + "/bbsdoc?board="+boardName+"&start=999999&type=doc";
            }
        }
    }
     ...
     * @return
     */
    public static String getThemeArticleDetailUrl(String board, String articleId, int page){
        GlobalStateSource gss = (GlobalStateSource) DataRepo.getInstance().getSource(SourceName.GLOBAL_STATE);
        if(gss.getLoginStatus() == GlobalStateSource.LOGIN_STATUS_NO_LOGIN){
            return bbsHost + "bbstcon?board="+board
                    +"&file="+articleId+"&start="+page;
        }else {
            return bbsHost + "vd" + gss.getBbsCode() + "/bbstcon?board="+board
                    +"&file="+articleId+"&start="+page;
        }
    }
     ...
     * @return
     */
    public static String getNormalArticleDetailUrl(String board, String articleId){
        GlobalStateSource gss = (GlobalStateSource) DataRepo.getInstance().getSource(SourceName.GLOBAL_STATE);
        if(gss.getLoginStatus() == GlobalStateSource.LOGIN_STATUS_NO_LOGIN){
            return bbsHost + "bbscon?board="+board
                    +"&file="+articleId+"&num=999999";// TIP 小技巧，num的值能获取最新的一般模式下的帖子数据
        }else {
            return bbsHost + "vd" + gss.getBbsCode() + "/bbscon?board="+board
     ...
     * @return
     */
    public static String searchArticleUrl() {
        GlobalStateSource gss = (GlobalStateSource) DataRepo.getInstance().getSource(SourceName.GLOBAL_STATE);
        if(gss.getLoginStatus() == GlobalStateSource.LOGIN_STATUS_NO_LOGIN){
            return bbsHost + "bbsfind";
        }else {
            return bbsHost + "vd" + gss.getBbsCode() + "/bbsfind";
        }
    }
     ...
     * @return
     */
    public static String getBbsUserInfoUrl(String username) {
        GlobalStateSource gss = (GlobalStateSource) DataRepo.getInstance().getSource(SourceName.GLOBAL_STATE);
        if(gss.getLoginStatus() == GlobalStateSource.LOGIN_STATUS_NO_LOGIN){
            return bbsHost + "bbsqry?userid=" + username;
        }else {
            return bbsHost + "vd" + gss.getBbsCode() + "/bbsqry?userid=" + username;
        }
    }
     ...
     * @return
     */
    public static String getZoneHotUrl(int sec) {
        if(sec < 0){
            sec = 0;
        }
        if(sec > 11){
        ...
        if(gss.getLoginStatus() == GlobalStateSource.LOGIN_STATUS_NO_LOGIN){
            ...
            return bbsHost + "vd" + gss.getBbsCode() + "/bbstop10s?sec=" + sec;
        }
    }
    ...
    public static String getBbsStatusUrl(){
        GlobalStateSource gss = (GlobalStateSource) DataRepo.getInstance().getSource(SourceName.GLOBAL_STATE);
        if(gss.getLoginStatus() == GlobalStateSource.LOGIN_STATUS_NO_LOGIN){
            return bbsHost + "bbsfoot";
        }else {
            return bbsHost + "vd" + gss.getBbsCode() + "/bbsfoot";
        }
    }
}
